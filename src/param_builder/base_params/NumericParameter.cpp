...
template <typename T>
const char NumericParameter<T>::NON_NUMERIC_CHAR_IN_PARAM_EXCEPTION_MSG_FORMAT_STRING[] = "ERROR! Character \"%c\" at index: %zu not valid";
...
template <typename T>
std::string NumericParameter<T>::getNonNumericCharacterInParamExceptionMessage(const char badChar, const size_t index)
{
    ...
    std::snprintf(buff, NON_NUMERIC_CHAR_IN_PARAM_EXCEPTION_MSG_MAX_SIZE,
            NON_NUMERIC_CHAR_IN_PARAM_EXCEPTION_MSG_FORMAT_STRING, badChar, index);
...
uint32_t NumericParameter<uint32_t>::stringToInt(const std::string& str)
{
    ...
    {
        throw std::invalid_argument(getNonNumericCharacterInParamExceptionMessage(str.c_str()[firstInvalidCharIdx],
                firstInvalidCharIdx));
    }
